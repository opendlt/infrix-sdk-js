...
test('ACTION_IDS matches the Go registry fixture (no drift)', () => {
  ...
  assert.equal(ACTION_IDS.length, 67, 'nextux-01 + nextux-02 + nextux-05 + nextux-06 + nextux-07 + nextux-08 + nextux-09 + nextux-10 + nextux-11 + nextux-12 + nextux-13 actions');
});
...
test('listActions parses the registry over the wire', async () => {
  ...
  assert.equal(actions.length, 67);
